...
import { Github, CheckCircle2, Loader2, ChevronDown, Zap, CalendarClock, Webhook, AlertCircle } from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      ...
      {connection.repo_full_name && (
        <div className="flex items-center gap-3 rounded-xl border border-zinc-700 bg-zinc-900/60 px-4 py-3">
          <Webhook className="h-4 w-4 text-emerald-400 shrink-0" />
          <div className="flex-1 min-w-0">
            <p className="text-xs text-zinc-500">Currently watching</p>
            <p className="text-sm font-medium text-white">{connection.repo_full_name}</p>
          </div>
          <Badge className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20 text-[10px] shrink-0">Active</Badge>
        </div>
      )}
      ...
      {!connection.repo_full_name && (
        <div className="flex items-center gap-3 rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-3">
          ...
          <p className="text-sm text-zinc-400">Select a repo below and click <span className="text-white font-medium">Save &amp; install webhook</span> to start auto-generating.</p>
        </div>
      )}
      ...
      <div className="border border-zinc-800 rounded-2xl overflow-hidden">
        <div className="px-5 py-4 bg-zinc-900/60 border-b border-zinc-800">
          <p className="text-sm font-semibold text-white">Repository to watch</p>
          <p className="text-xs text-zinc-500 mt-0.5">
            Listens for <code className="text-zinc-400 bg-zinc-800 px-1 rounded">feat:</code> / <code className="text-zinc-400 bg-zinc-800 px-1 rounded">fix:</code> commits on the main branch.
          </p>
        </div>
        <div className="p-4 space-y-4">
            ...
            <label className="flex items-center justify-between rounded-xl border border-zinc-800 bg-zinc-900/60 px-4 py-3 cursor-pointer hover:border-zinc-700 transition-colors">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500">Create posts automatically on qualifying commits</p>
                </div>
              </div>
              <div
            ...
            <label className="flex items-center justify-between rounded-xl border border-zinc-800 bg-zinc-900/60 px-4 py-3 cursor-pointer hover:border-zinc-700 transition-colors">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500">Queue the tweet for the next open slot (Studio plan)</p>
                </div>
              </div>
              <div
          ...
          >
            {saving
              ...
              : <><Github className="h-4 w-4 mr-2" />{connection.repo_full_name ? "Update settings & reinstall webhook" : "Save & install webhook"}</>
            }
          </Button>
        </div>
      </div>
